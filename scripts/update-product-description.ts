 ...
 *
 * This script updates the TLDW Pro product description to show the correct
...
/**
 * Find the TLDW Pro product
 */
async function findTLDWProProduct(stripe: Stripe): Promise<Stripe.Product | null> {
  try {
    const products = await stripe.products.list({ limit: 100 });
    const proProduct = products.data.find(
      p => p.name.includes('TLDW Pro') || p.name.includes('Pro')
    );
...
async function main() {
  ...
  console.log('This script will update the TLDW Pro product description.\n');
  ...
  // Find the TLDW Pro product
  console.log('🔍 Searching for TLDW Pro product...\n');
  const product = await findTLDWProProduct(stripe);

  if (!product) {
    console.error('❌ Error: Could not find TLDW Pro product');
